...
async function identify_user(agent)
{
  ...
  const snap = await client.db("details").collection("user").findOne({acct_num: acct_num});
  
  if(snap==null){
	  await agent.add("Please check your account number");
...
function report_issue(agent)
{
  ...
  var dbo = db.db("details");
    ...
    var issue_val=  val;     //function scope
    var status="pending";

	let ts = Date.now();
    let date_ob = new Date(ts);    //block scope
...
intentMap.set("services_intent - custom - custom", report_issue);
